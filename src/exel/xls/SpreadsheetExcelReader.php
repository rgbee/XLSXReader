...
use socialist\exel\xls\OLERead;
...
class SpreadsheetExcelReader extends OLERead implements ExelReaderInterface
{
	...
	public function __construct($file, $store_extended_info = true, $outputEncoding = '')
	{
		...
		for ($i = 1; $i < 245; $i++) {
			$name = strtolower(
					( ( ($i - 1) / 26 >= 1 ) ? chr( ($i - 1) / 26 + 64 ) : '' ) . chr( ($i - 1) % 26 + 65)
				);
	...
	{
		if(array_key_exists($sheetIndex, $this->boundsheets)) {
			return $this->boundsheets[$sheetIndex]['name'];
		}
		return null;
	}
	...
	public function getNumRows($sheetIndex = 0)
	{
		if(array_key_exists($sheetIndex, $this->sheets)) {
			return $this->sheets[$sheetIndex]['numRows'];
		}
		return null;
	}
	...
	{
		if(array_key_exists($sheetIndex, $this->sheets)) {
			return $this->sheets[$sheetIndex]['numCols'];
		}
		return null;
	}
	...
	{
		return $this->value($row, $col, $sheet);
	}
	...
	{
		if(array_key_exists($rowNum, $this->sheets[$sheetIndex]['cells'])) {
			return $this->sheets[$sheetIndex]['cells'][$rowNum];
		}
		return [];
	}
	 ...
	 * 
	 * @param  boolean $row_numbers [description]
	 * @param  boolean $col_letters [description]
	...
	private function myHex($d)
	{
		if ($d < 16) return "0" . dechex($d);
		return dechex($d);
	}

	private function dumpHexData($data, $pos, $length)
	{
		$info = "";
	...
	private function _parse()
	{
		...
		$code = ExelReaderHelper::v($data,$pos);
		$length = ExelReaderHelper::v($data,$pos+2);
		$version = ExelReaderHelper::v($data,$pos+4);
		$substreamType = ExelReaderHelper::v($data,$pos+6);
			...
			switch ($code) {
				...
				case self::SPREADSHEET_EXCEL_READER_TYPE_SST:
					...
					for ($i = 0; $i < $uniqueStrings; $i++) {
						...
						if ($spos == $limitpos) {
							$opcode = ExelReaderHelper::v($data, $spos);
							$conlength = ExelReaderHelper::v($data, $spos+2);
							if ($opcode != 0x3c) {
								return -1;
							}
							$spos += 4;
						...
						if ($richString) {
							// Read in the crun
							$formattingRuns = ExelReaderHelper::v($data, $spos);
							$spos += 2;
						}
				...
				case self::SPREADSHEET_EXCEL_READER_TYPE_FORMAT:
					$indexCode = ExelReaderHelper::v($data,$pos+4);

					if ($version == self::SPREADSHEET_EXCEL_READER_BIFF8) {
						$numchars = ExelReaderHelper::v($data,$pos+6);
						if (ord($data[$pos+8]) == 0){
				...
				case self::SPREADSHEET_EXCEL_READER_TYPE_FONT:
						...
						$height = ExelReaderHelper::v($data,$pos+4);
						$option = ExelReaderHelper::v($data,$pos+6);
						$color = ExelReaderHelper::v($data,$pos+8);
						$weight = ExelReaderHelper::v($data,$pos+10);
				...
				case self::SPREADSHEET_EXCEL_READER_TYPE_PALETTE:
						...
						for ($coli = 0; $coli < $colors; $coli++) {
							...
							$this->colors[0x07 + $coli] = '#' . $this->myhex($colr) .
														  $this->myhex($colg) .
														  $this->myhex($colb);
						}
					    break;
...
//						
	...
	private function read16bitstring($data, $start)
	{
		$len = 0;
